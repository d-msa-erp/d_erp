...
let totalPages = 1;
const pageSize = 10;
let currentInvTransIdxForModal = null; // 현재 모달에서 열린 출고 트랜잭션 ID
...
// === UI 컨트롤 요소 가져오기 ===
const outboundTableBody = document.querySelector('#outboundTable tbody'); // 테이블 ID 변경
...
const modal = document.getElementById('outboundModal'); // 모달 ID 변경
...
async function loadOutboundTable(page = currentPage, sortBy = currentSortBy, sortDirection = currentOrder, filters = searchFilters) {
    ...
    displayNoDataMessage(outboundTableBody, 11); // 컬럼 수에 맞게 조정
    ...
    queryParams.append('transType', 'S'); // 출고는 'S'로 고정
    ...
    try {
        const response = await fetch(`/api/inv-transactions?${queryParams.toString()}`); // API 엔드포인트 유지
        if (!response.ok) {
            const errorText = await response.text();
            console.error(`Error fetching outbound list: HTTP status ${response.status}`, errorText);
        ...
        outboundTableBody.innerHTML = ''; // 데이터가 있을 경우 다시 초기화
        ...
        displayNoDataMessage(outboundTableBody, 11, true); // 컬럼 수에 맞게 조정
    }
}
...
// === 테이블 정렬 함수 ===
document.querySelectorAll('#outboundTable thead th').forEach(th => { // 테이블 ID 변경
...
function order(thElement) {
    ...
    document.querySelectorAll('#outboundTable thead th .sort-arrow').forEach(arrow => { // 테이블 ID 변경
        arrow.textContent = '↓';
...
async function openModal(mode, invTransIdx = null) {
    ...
    modalTitle.textContent = '신규 출고 등록'; // 텍스트 변경
    ...
    if (mode === 'new') {
        ...
        modalTitle.textContent = '출고 상세 정보'; // 텍스트 변경
        ...
        try {
            ...
            console.error('출고 상세 정보를 불러오는 중 오류 발생:', error); // 텍스트 변경
            alert('출고 정보를 불러오는데 실패했습니다.'); // 텍스트 변경
            closeModal();
            return;
        }
    } else {
...
function outsideClick(e) {
    if (e.target.id === 'outboundModal') closeModal(); // 모달 ID 변경
}
...
function getTransStatusText(statusCode) {
    switch (statusCode) {
        ...
        case 'S2':
            return '출고완료';
        case 'R1': // 입고 상태도 포함 (API에서 같이 올 수 있으므로)
            return '입고전';
        case 'R2':
            return '가입고';
        case 'R3':
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadOutboundTable(); // 페이지 로드 시 출고 목록 로드
    ...
    searchButton.addEventListener('click', (event) => {
        ...
        loadOutboundTable(1, currentSortBy, currentOrder, searchFilters); // 검색 시 1페이지로 이동
    });
    ...
    resetSearchButton.addEventListener('click', () => {
        // 검색 필드 값 초기화
        document.getElementById('outboundForm').reset(); // 폼 ID 변경
        ...
        loadOutboundTable(1); // 초기화 후 1페이지로 다시 로드
    });
    ...
    outboundTableBody.addEventListener('change', function(event) { // 테이블 바디 ID 변경
    ...
    modalForm.addEventListener('submit', async (event) => {
        ...
        if (!modalHiddenWhIdxInput.value) {
            alert("유효한 출고 창고를 선택해주세요."); // 텍스트 변경
        ...
        const data = {
            ...
            transStatus: formData.get('transStatus') || 'S1', // 신규 등록 시 '출고전' 기본값 (R1 -> S1로 변경)
        ...
        try {
            ...
            alert(isEditMode ? '출고 정보가 성공적으로 수정되었습니다.' : '새 출고가 성공적으로 등록되었습니다.'); // 텍스트 변경
            closeModal();
            loadOutboundTable(currentPage, currentSortBy, currentOrder, searchFilters); // 현재 검색/정렬 상태 유지하며 목록 새로고침
        } catch (error) {
            console.error('Error saving outbound data:', error); // 텍스트 변경
            alert(`출고 ${isEditMode ? '수정' : '등록'}에 실패했습니다: ${error.message}`); // 텍스트 변경
        }
    });
    ...
    deleteButton.addEventListener('click', async () => {
        ...
        if (invTransIdxesToDelete.length === 0) {
            alert('삭제할 출고 항목을 선택해주세요.'); // 텍스트 변경
            return;
        }

        if (!confirm(`${invTransIdxesToDelete.length}개의 출고 항목을 정말로 삭제하시겠습니까?`)) { // 텍스트 변경
            return;
        }
        ...
        try {
            ...
            alert('선택된 출고 항목이 성공적으로 삭제되었습니다.'); // 텍스트 변경
            loadOutboundTable(currentPage, currentSortBy, currentOrder, searchFilters); // 현재 검색/정렬 상태 유지하며 목록 새로고침
        } catch (error) {
            console.error('Error deleting outbound data:', error); // 텍스트 변경
            alert(`출고 항목 삭제에 실패했습니다: ${error.message}`); // 텍스트 변경
        }
    });
