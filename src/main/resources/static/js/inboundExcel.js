...
async function downloadSelectedInboundDetailsAsExcel() {
    ...
    if (checkedCheckboxes.length === 0) {
        alert("엑셀로 다운로드할 입고 항목을 먼저 선택해주세요.");
        return;
    }
    ...
    try {
        const response = await fetch('/api/inv-transactions/download-excel-details', { // 입고/출고 공용 API 엔드포인트 사용 가능성
            ...
            body: JSON.stringify({ invTransIds: selectedInvTransIds, transType: 'R' }), // transType 'R' (입고) 명시
        });
        ...
        let filename = 'inbound_details.xlsx'; // 기본 파일명
