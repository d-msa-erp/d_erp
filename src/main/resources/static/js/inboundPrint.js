...
async function printSelectedInboundDetails() {
    ...
    let printContents = `
        ...
        <body>
            <h1 class="print-main-title">선택된 입고 상세 정보</h1>
    `;
    ...
    try {
        const transactionDetailsPromises = selectedInvTransIds.map(id =>
            fetch(`/api/inv-transactions/${id}`) // 개별 거래 상세 정보를 가져올 API 엔드포인트
                .then(res => {
                    if (!res.ok) {
                        console.error(`[Print] 입고 거래 ID ${id} 상세 정보 조회 실패: ${res.status}`);
                        return { id, error: true, statusText: res.statusText, status: res.status };
                    }
                    return res.json();
                })
                .catch(err => {
                    console.error(`[Print] 입고 거래 ID ${id} fetch 중 네트워크 오류 또는 JSON 파싱 오류:`, err);
        ...
        results.forEach(transactionData => {
            if (!transactionData || transactionData.error) {
                printContents += `
                    <div class="transaction-container">
                        <h2>입고 거래 ID: ${transactionData.id || '알 수 없음'}</h2>
                        <p style="color:red;">이 입고 거래의 상세 정보를 불러오는 데 실패했습니다. (오류: ${transactionData.statusText || '데이터 없음'})</p>
                    </div>`;
                return;
            }
            ...
            printContents += `<h2>입고 코드: ${transactionData.invTransCode || 'N/A'}</h2>`;
            printContents += `<div class="detail-info">`;
            printContents += `<p><strong>입고일:</strong> ${formatDate(transactionData.transDate) || 'N/A'}</p>`;
            ...
            printContents += `<p><strong>입고수량:</strong> ${transactionData.transQty !== null ? Number(transactionData.transQty).toLocaleString() : '0'}</p>`;
            ...
            printContents += `<p><strong>입고창고:</strong> ${transactionData.whNm || 'N/A'}</p>`;
            printContents += `<p><strong>입고관리자:</strong> ${transactionData.userNm || '미지정'}</p>`;
        ...
        console.error("[Print] 입고 상세 정보 처리 중 전체 오류:", error);
        printContents += `<div class="transaction-container"><p style="color:red;">선택된 입고 정보를 처리하는 중 예기치 않은 오류가 발생했습니다: ${error.message}</p></div>`;
    }
...
// === 날짜 및 상태 형식화 헬퍼 함수 (inbound.js에서 가져옴) ===
